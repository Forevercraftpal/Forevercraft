...
tellraw @s ["",{text:"[Forevercraft] ",color:"gold"},{text:"Task Complete! ",color:"aqua"},{text:"+100 XP ",color:"green"},{text:"+75 REP ",color:"aqua"},{text:"+2 Coins",color:"#E0B0FF"}]
...
scoreboard players set #rp_quest_tier rp.temp 2
...
# Forever Coin reward: Task = 2 coins (no DR scaling)
scoreboard players add @s ec.coins 2
execute if score @s ec.guild_id matches 1.. run function evercraft:guild/contribution/add {amount:10}

# Guild Expedition: Quest bonus XP (activity=7)
function evercraft:guild/expedition/bonus_xp {base:100,activity:7}
