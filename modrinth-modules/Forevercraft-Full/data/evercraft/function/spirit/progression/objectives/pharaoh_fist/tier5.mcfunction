# Pharaoh's Fist — Ornate → Exquisite (8 objectives)
# 1. Kill 5,000 mobs
execute if score @s ec.sp_kills matches 5000.. run scoreboard players add @s ec.sp_obj_done 1
# 2. Use Titan Breaker 200 times
execute if score @s ec.sp_ability1_uses matches 200.. run scoreboard players add @s ec.sp_obj_done 1
# 3. Land 2,000 impact strikes
execute if score @s ec.sp_impact_hits matches 2000.. run scoreboard players add @s ec.sp_obj_done 1
...
# 8. Win 20 duels
execute if score @s ec.sp_duel_wins matches 20.. run scoreboard players add @s ec.sp_obj_done 1
