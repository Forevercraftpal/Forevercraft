# Pharaoh's Fist — Mythical → Spirit (10 objectives)
# 1. Kill 50,000 mobs
execute if score @s ec.sp_kills matches 50000.. run scoreboard players add @s ec.sp_obj_done 1
# 2. Land 15,000 impact strikes
execute if score @s ec.sp_impact_hits matches 15000.. run scoreboard players add @s ec.sp_obj_done 1
...
# 7. Win 50 duels
execute if score @s ec.sp_duel_wins matches 50.. run scoreboard players add @s ec.sp_obj_done 1
