# Bulwark Shield — Rare → Ornate (7 objectives)
# 1. Kill 2,000 mobs
execute if score @s ec.sp_kills matches 2000.. run scoreboard players add @s ec.sp_obj_done 1
# 2. Use Fortress Wall 50 times
execute if score @s ec.sp_ability1_uses matches 50.. run scoreboard players add @s ec.sp_obj_done 1
# 3. Absorb 10,000 damage
execute if score @s ec.sp_total_absorb matches 10000.. run scoreboard players add @s ec.sp_obj_done 1
