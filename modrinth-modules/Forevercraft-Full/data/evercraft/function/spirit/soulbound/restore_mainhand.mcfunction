# Soulbound — Restore mainhand spirit artifact from per-player vault
# Find vault marker matching this player's uid
scoreboard players operation #owner_check ec.temp = @s ec.sp_uid

# Copy vault's stored mainhand to temp storage for hopper minecart transfer
execute as @e[tag=ec.sp_vault] if score @s ec.sp_owner = #owner_check ec.temp run data modify storage evercraft:temp sb_restore set from entity @s data.mainhand
