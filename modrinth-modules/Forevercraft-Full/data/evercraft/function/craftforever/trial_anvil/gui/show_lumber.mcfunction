# Trial Anvil — Show Lumber Trial tier selection
...
execute as @e[type=text_display,tag=TT.Title,distance=..7] if score @s adv.gui_session = #gui_check ec.temp run data modify entity @s text set value {"text":"\u2042 Lumber Trials","color":"dark_green","bold":true}

scoreboard players operation @s ec.tt_tier = @s ec.tt_lumb_best
scoreboard players add @s ec.tt_tier 1
execute if score @s ec.tt_tier matches 11.. run scoreboard players set @s ec.tt_tier 10
scoreboard players set @s ec.tt_type 5
...
execute store result storage evercraft:trials best int 1 run scoreboard players get @s ec.tt_lumb_best
...
function evercraft:craftforever/trial_anvil/gui/set_lumber_desc
