# Housing — Return to overview page (runs as player)
...
# Restore Housing title
execute as @e[type=marker,tag=CF.MenuAnchor,distance=..5,limit=1,sort=nearest] if score @s ec.cf_gui_session = #cf_gui_stamp ec.temp at @s run data modify entity @e[type=text_display,tag=CF.MenuTitle,distance=..5,limit=1] text set value {"text":"\u2726 Housing \u2726","color":"#66BB6A","bold":true}

# Re-spawn overview
function evercraft:craftforever/codex/hub/housing/spawn_overview
