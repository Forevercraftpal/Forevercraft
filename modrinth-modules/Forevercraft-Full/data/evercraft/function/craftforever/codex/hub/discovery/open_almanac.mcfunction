# Discovery — Open Almanac sub-page (runs as player)
...
execute as @e[type=marker,tag=CF.MenuAnchor,distance=..5] if score @s ec.cf_gui_session = #cf_gui_stamp ec.temp at @s run data modify entity @e[type=text_display,tag=CF.MenuTitle,distance=..5,limit=1] text set value {text:"\u2726 Almanac \u2726",color:"#4FC3F7",bold:true}

function evercraft:craftforever/codex/hub/almanac/spawn
