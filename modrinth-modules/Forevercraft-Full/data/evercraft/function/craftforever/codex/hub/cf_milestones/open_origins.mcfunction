# CF Milestones — Open Origins detail page (runs as player)
...
# Set page (9 = Origins, matching FC codex)
scoreboard players set @s ec.cf_codex_page 9

# Update title
execute as @e[type=marker,tag=CF.MenuAnchor,distance=..5] if score @s ec.cf_gui_session = #cf_gui_stamp ec.temp at @s run data modify entity @e[type=text_display,tag=CF.MenuTitle,distance=..5,limit=1] text set value {"text":"\u2726 Origins \u2726","color":"yellow","bold":true}

# Spawn FC milestone detail page (creates entities with Adv.* tags)
function evercraft:codex/hub/journal/spawn_ms_origins
