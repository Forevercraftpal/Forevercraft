# ============================================================
# Artifact Bounty Board — Turn In Slot 0
# Run as player. Consumes artifact, grants coins + random item
# ============================================================
...
# Mark claimed
scoreboard players set @s ec.abb_claim_0 1

# Copy reward data to temp
data modify storage eden:database bounty_board.temp.coins set from storage eden:database bounty_board.bounties[0].coins
data modify storage eden:database bounty_board.temp.name set from storage eden:database bounty_board.bounties[0].name
data modify storage eden:database bounty_board.temp.tier set from storage eden:database bounty_board.bounties[0].tier

# Remove artifact from hand
...
# Announce coin reward
function evercraft:bounty_board/announce with storage eden:database bounty_board.temp

# Grant random niche fountain item (tier-matched, capped at exquisite)
