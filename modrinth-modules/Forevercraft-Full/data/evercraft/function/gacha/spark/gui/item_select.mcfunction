# Lucid Claim — Artifact Selection Handler (Macro)
...
$execute if entity @e[type=text_display,tag=ec.Confirming,tag=Adv.ArtI_$(art),distance=..10,limit=1] run function evercraft:gacha/spark/gui/grant {art:"$(art)",tier:"$(tier)"}
...
function evercraft:gacha/spark/gui/reset_highlight

# Save the artifact's display text before overwriting
$execute as @e[type=text_display,tag=Adv.ArtI_$(art),distance=..10,limit=1] run data modify storage evercraft:lucid_claim selected_name set from entity @s text
