# Dragonheart Sword — Exquisite → Mythical (9 objectives)
# 1. Kill 15,000 mobs
execute if score @s ec.sp_kills matches 15000.. run scoreboard players add @s ec.sp_obj_done 1
# 2. Use Dragon Strike 500 times
execute if score @s ec.sp_ability1_uses matches 500.. run scoreboard players add @s ec.sp_obj_done 1
# 3. Block 15,000 attacks
execute if score @s ec.sp_shield_block matches 15000.. run scoreboard players add @s ec.sp_obj_done 1
