# Bulwark Shield — Exquisite → Mythical (9 objectives)
# 1. Kill 15,000 mobs
execute if score @s ec.sp_kills matches 15000.. run scoreboard players add @s ec.sp_obj_done 1
# 2. Use Fortress Wall 500 times
execute if score @s ec.sp_ability1_uses matches 500.. run scoreboard players add @s ec.sp_obj_done 1
# 3. Absorb 75,000 damage
execute if score @s ec.sp_total_absorb matches 75000.. run scoreboard players add @s ec.sp_obj_done 1
