# Ellegaard Trident — Mythical → Spirit (10 objectives)
# 1. Kill 50,000 mobs
execute if score @s ec.sp_kills matches 50000.. run scoreboard players add @s ec.sp_obj_done 1
# 2. Get 15,000 thrown kills
execute if score @s ec.sp_thrown_kills matches 15000.. run scoreboard players add @s ec.sp_obj_done 1
...
# 7. Explore 20 ocean structures
execute if score @s ec.sp_ocean_explore matches 20.. run scoreboard players add @s ec.sp_obj_done 1
