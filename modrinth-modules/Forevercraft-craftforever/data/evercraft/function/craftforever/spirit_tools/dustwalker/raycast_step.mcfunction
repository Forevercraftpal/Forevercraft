# Dustwalker — Raycast step (max 12 blocks = 24 steps)
execute if score #st_ray ec.temp matches 24.. run return 0

# Hit shovel-mineable → place marker
execute unless block ~ ~ ~ air unless block ~ ~ ~ cave_air unless block ~ ~ ~ void_air unless block ~ ~ ~ water unless block ~ ~ ~ lava if predicate evercraft:mythical_tools/is_shovel_mineable align xyz positioned ~0.5 ~0.5 ~0.5 run summon marker ~ ~ ~ {Tags:["ec.st_shov_target"]}
...
execute positioned ^ ^ ^0.5 run function evercraft:craftforever/spirit_tools/dustwalker/raycast_step
