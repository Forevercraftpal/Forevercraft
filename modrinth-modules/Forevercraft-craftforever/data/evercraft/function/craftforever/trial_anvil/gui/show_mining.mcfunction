# Trial Anvil — Show Mining Trial tier selection
# Replaces category view with mining-specific tier info
...
# Update title
execute as @e[type=text_display,tag=TT.Title,distance=..7] if score @s adv.gui_session = #gui_check ec.temp run data modify entity @s text set value {"text":"\u26cf Mining Trials","color":"#FFA500","bold":true}

# Calculate next tier (best + 1, capped at 10)
scoreboard players operation @s ec.tt_tier = @s ec.tt_m_best
...
# Show tier progress via macro
execute store result storage evercraft:trials best int 1 run scoreboard players get @s ec.tt_m_best
...
# Tier description (shows what the next tier challenge is)
...
# Fill in tier-specific descriptions
function evercraft:craftforever/trial_anvil/gui/set_mining_desc

# Session tag new entities
...
# Session tag new entities again
