# Spirit Raids — Boss 5: The Gatekeeper (Stronghold)
...
# Set boss ID
scoreboard players set #rd_boss_id ec.var 5

# Summon — wither skeleton base (dual swords, tall, menacing)
summon wither_skeleton ~ ~ ~ {Tags:["ec.rd_boss","ec.rd_gk"],NoAI:0b,Silent:1b,PersistenceRequired:1b,equipment:{mainhand:{id:"minecraft:iron_sword"},offhand:{id:"minecraft:iron_sword"}}}

# Compute final HP: base 1700 × hp_scale / 100 (850 hearts)
scoreboard players set #rd_temp ec.var 1700
...
# Setup boss macro
data modify storage evercraft:raids temp set value {boss_name:"The Gatekeeper",boss_color:"dark_green",base_armor:16,base_speed:0.28d,base_damage:6.0d,boss_scale:1.5d,bar_color:"green"}
...
scoreboard players set #rd_gk_whirl_cd ec.var 0
scoreboard players set #rd_gk_fire_cd ec.var 0

# Boss intro
tellraw @a[tag=ec.rd_participant] [{text:""},{"text":"══════════════════════════════","color":"dark_green"}]
tellraw @a[tag=ec.rd_participant] [{text:"  💀 "},{"text":"The Gatekeeper","color":"dark_green","bold":true},{"text":" draws twin blades from the void...","color":"gray"}]
tellraw @a[tag=ec.rd_participant] [{text:"  "},{"text":"\"None pass. None have. None will.\"","color":"green","italic":true}]
tellraw @a[tag=ec.rd_participant] [{text:"══════════════════════════════","color":"dark_green"}]
