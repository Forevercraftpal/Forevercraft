# Chrono Shard — select Personal Milestones
# @s = interaction entity — route to owner via ec.cs_owner
data remove entity @s interaction
scoreboard players operation #cs_owner ec.temp = @s ec.cs_owner
execute as @a[scores={ec.cs_active=1}] if score @s ec.cs_owner = #cs_owner ec.temp run scoreboard players set @s ec.cs_select 6
