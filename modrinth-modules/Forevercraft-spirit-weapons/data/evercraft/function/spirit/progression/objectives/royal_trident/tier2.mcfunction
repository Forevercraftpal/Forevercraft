# Depthsworn — Common → Uncommon (5 objectives)
# 1. Kill 100 mobs
execute if score @s ec.sp_kills matches 100.. run scoreboard players add @s ec.sp_obj_done 1
# 2. Land 25 counter thrust hits
execute if score @s ec.sp_counter_hits matches 25.. run scoreboard players add @s ec.sp_obj_done 1
# 3. Visit 5 biomes
