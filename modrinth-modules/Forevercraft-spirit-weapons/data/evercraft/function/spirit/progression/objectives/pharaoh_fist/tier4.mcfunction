# Pharaoh's Fist — Rare → Ornate (7 objectives)
# 1. Kill 2,000 mobs
execute if score @s ec.sp_kills matches 2000.. run scoreboard players add @s ec.sp_obj_done 1
# 2. Use Titan Breaker 50 times
execute if score @s ec.sp_ability1_uses matches 50.. run scoreboard players add @s ec.sp_obj_done 1
# 3. Land 500 impact strikes
execute if score @s ec.sp_impact_hits matches 500.. run scoreboard players add @s ec.sp_obj_done 1
