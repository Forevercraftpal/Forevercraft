# Ellegaard Trident — Rare → Ornate (7 objectives)
# 1. Kill 2,000 mobs
...
# 3. Get 500 thrown kills
execute if score @s ec.sp_thrown_kills matches 500.. run scoreboard players add @s ec.sp_obj_done 1
...
# 7. Prestige Explorer
execute if score @s adv.pres_expl matches 1.. run scoreboard players add @s ec.sp_obj_done 1
