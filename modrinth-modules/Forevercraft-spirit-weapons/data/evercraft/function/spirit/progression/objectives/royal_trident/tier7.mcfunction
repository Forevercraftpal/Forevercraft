# Depthsworn — Mythical → Spirit (10 objectives)
# 1. Kill 50,000 mobs
execute if score @s ec.sp_kills matches 50000.. run scoreboard players add @s ec.sp_obj_done 1
# 2. Land 15,000 counter thrust hits
execute if score @s ec.sp_counter_hits matches 15000.. run scoreboard players add @s ec.sp_obj_done 1
...
# 7. Reach Castle floor 40 as Hoplite
execute if score @s ec.sp_castle_hoplite matches 1.. run scoreboard players add @s ec.sp_obj_done 1
