# Depthsworn — Ornate → Exquisite (8 objectives)
# 1. Kill 5,000 mobs
execute if score @s ec.sp_kills matches 5000.. run scoreboard players add @s ec.sp_obj_done 1
# 2. Use Counter Thrust 200 times
execute if score @s ec.sp_ability1_uses matches 200.. run scoreboard players add @s ec.sp_obj_done 1
# 3. Land 2,000 counter thrust hits
execute if score @s ec.sp_counter_hits matches 2000.. run scoreboard players add @s ec.sp_obj_done 1
...
# 8. Block 2,000 attacks
execute if score @s ec.sp_shield_block matches 2000.. run scoreboard players add @s ec.sp_obj_done 1
