# Bulwark Shield — Ornate → Exquisite (8 objectives)
# 1. Kill 5,000 mobs
execute if score @s ec.sp_kills matches 5000.. run scoreboard players add @s ec.sp_obj_done 1
# 2. Use Fortress Wall 200 times
execute if score @s ec.sp_ability1_uses matches 200.. run scoreboard players add @s ec.sp_obj_done 1
# 3. Absorb 25,000 damage
execute if score @s ec.sp_total_absorb matches 25000.. run scoreboard players add @s ec.sp_obj_done 1
...
# 8. Survive 2 deathless hours
execute if score @s ec.sp_deathless_hours matches 2.. run scoreboard players add @s ec.sp_obj_done 1
