# Dragonheart Sword — Ornate → Exquisite (8 objectives)
# 1. Kill 5,000 mobs
execute if score @s ec.sp_kills matches 5000.. run scoreboard players add @s ec.sp_obj_done 1
# 2. Use Dragon Strike 200 times
execute if score @s ec.sp_ability1_uses matches 200.. run scoreboard players add @s ec.sp_obj_done 1
# 3. Block 5,000 attacks
execute if score @s ec.sp_shield_block matches 5000.. run scoreboard players add @s ec.sp_obj_done 1
...
# 8. Protect allies from 2,000 damage
execute if score @s ec.sp_ally_protect matches 2000.. run scoreboard players add @s ec.sp_obj_done 1
