# Pharaoh's Fist — Exquisite → Mythical (9 objectives)
# 1. Kill 15,000 mobs
execute if score @s ec.sp_kills matches 15000.. run scoreboard players add @s ec.sp_obj_done 1
# 2. Use Titan Breaker 500 times
execute if score @s ec.sp_ability1_uses matches 500.. run scoreboard players add @s ec.sp_obj_done 1
# 3. Land 5,000 impact strikes
execute if score @s ec.sp_impact_hits matches 5000.. run scoreboard players add @s ec.sp_obj_done 1
